  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      this.issueService.getIssueById(this.id).subscribe(res => {
        this.issue = res;
        console.log(this.issue[0]);
        this.updateForm.get('name').setValue(this.issue[0].name);
        this.updateForm.get('price').setValue(this.issue[0].price);
        this.updateForm.get('description').setValue(this.issue[0].description);
        this.updateForm.get('type').setValue(this.issue[0].type);
      });
    });
  }
